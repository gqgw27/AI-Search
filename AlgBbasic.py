...
def crossover(P1,P2):
    c1 = P1.copy()
    c2 = P2.copy()
    p = random.randint(0,len(P1)-2)
    for i in range(p+1):
        current = c1[i]
        new = P2[i]
        c1[c1.index(new)] = current
        c1[i] = new

    for i in range(p+1):
        current = c2[i]
        new = P1[i]
        c2[c2.index(new)] = current
        c2[i] = new
...
for i in range(P_Size):
    ...
    POP.append(T)
    Fitness.append(1/check_tour_length(T,distance_matrix))
...
for rep in range(Gen):
    ...
    PROB = [0]
    for f in Fitness:
        PROB.append(PROB[-1]+SUM*f) # Probability distribution
    ...
    for i in range(P_Size):
        ...
        l1 = check_tour_length(child1,distance_matrix)
        l2 = check_tour_length(child2,distance_matrix)

        bst,small = best(child1,child2,l1,l2)
...
tour = BEST
tour_length = check_tour_length(tour,distance_matrix)
