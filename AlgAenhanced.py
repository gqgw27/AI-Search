...
def two_opt(T):
    ...
    for rep in range(l):
        ...
        for i in range(l-1):
            ...
            s2 = distance_matrix[T[i+1]][T[(i+2)%l]]
            for j in range(i+1,l-1):
                ...
                if j == i+2:
                    ...
                    a2 = distance_matrix[T[(i+2)%l]][T[j]]
...
tour = []
tour_length = -1
# Repeatedly performing greedy, changing start city
for first in range(num_cities):
    tur,tur_length = greedy(first,distance_matrix)
    tur_length += two_opt(tur)
    if tour_length == -1:
        tour = tur
        tour_length = tur_length
    elif tur_length < tour_length:
        tour = tur
        tour_length = tur_length


tour_length += two_opt(tour)
tour_length = check_length(tour)
